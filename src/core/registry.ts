...
function assertNotSealed(action: string) {
  if (sealed)
    throw new Error(`Registry is sealed; cannot ${action} after database initialization.`);
}
...
export function ensureEntityMeta(target: Ctor, name?: string): EntityMeta {
  ...
  if (!meta) {
    ...
    if (conflict && conflict.target !== target) {
      throw new Error(
        `Duplicate table name detected: "${name}" is already registered for a different entity.`
      );
    }
    if (isDefaultOld) {
...
export function addColumn(
  ...
  if (!ctor) {
    throw new Error(
      `@Column used on an invalid target: missing constructor for property "${String(
        propertyKey
      )}".`
    );
  }
  ...
  if (options.autoIncrement && !options.primary) {
    throw new Error(
      `autoIncrement requires primary: true (table: ${meta.tableName}, column: ${key})`
    );
  }
...
export function resolveEntity(meta: EntityMeta): { columns: ColumnMeta[] } {
  ...
  if (distinctPrimaryKeys.size > 1) {
    throw new Error(
      `Multiple primary columns across inheritance for table "${meta.tableName}": ${[
        ...distinctPrimaryKeys,
      ].join(', ')}`
    );
  }
